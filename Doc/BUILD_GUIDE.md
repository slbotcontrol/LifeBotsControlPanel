# Creating a LifeBots Control Panel Release version
...
Once you have the required tools, edit the Makefile in the LifeBotsControlPanel folder to suit your needs, then using a terminal (also called command interpreter, shell, console... depending on the operating system) change to that folder using `cd <path-to-folder>` and type `make`. That should generate a file called `LifeBotsControlPanel.zip` with the packaged version for Second Life.

The optimized files will have an `.lslo` extension, and they are ready to be copied and pasted each into a Second Life script.
